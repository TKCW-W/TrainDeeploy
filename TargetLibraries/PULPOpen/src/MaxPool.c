...
/* Fused ReLU-MaxPool backward: pInput is the *pre-ReLU* activation (the
 * BatchNorm output). ReLU (max(0,.)) is applied inline while recomputing the
 * argmax, so the forward ReLU output X = ReLU(pInput) never needs to be stored.
 * Produces the identical dX as PULP_MaxPoolGrad2d fed the materialised X:
 * argmax over max(0, pInput) == argmax over X, and dX is grad w.r.t. X. */
