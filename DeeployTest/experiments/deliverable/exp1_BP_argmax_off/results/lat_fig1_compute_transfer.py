...
fig,ax=plt.subplots(figsize=(7.6,5.4))   # ~2/3 of the 12-in two-panel
...
ax.set_xticks([0]); ax.set_xticklabels(["one training step"]); ax.set_xlim(-.55,.55)
ax.set_ylabel("Cycles  (millions)"); ax.set_ylim(0,36)
ax.yaxis.grid(True, color="#e6e6e6", linewidth=.8); ax.set_axisbelow(True)
for s in ["top","right","bottom","left"]: ax.spines[s].set_visible(True)
ax.set_title("Compute vs transfer", fontsize=15, fontweight="bold", pad=12)
...
print("saved lat_fig1_compute_transfer.png")
