...
def run_zo_codegen(config, script_dir: Path) -> None:  # -- QW
    ...
    # --- ZO graphs are now emitted BP-native by Onnx4Deeploy (weights-as-inputs,
    # BatchNormInternal, canonical 2-output SCE, no mezo domain), so they flow
    # through the EXISTING training codegen with NO preprocessing — same as BP.
    # (The old zo_graph_prep.py downstream translation is removed.) -- QW
    train_dir = config.test_dir  # -- QW  raw native zo_train dir
