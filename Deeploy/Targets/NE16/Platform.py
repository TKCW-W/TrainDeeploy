...
class NE16Platform(GAP9Platform):
    ...
    def __init__(self,
        ...
        if engines is None:
            # QW: upstream PR #183 passes an explicit trimmed includeList here to keep the
            #     GAP9-SDK NE16 headers (CNN_BasicKernels_NE16.h / ne16_utils.h) out of the
            #     generated Network.c, because they redefine the NE16_REG_* macros that
            #     pulp-nnx's ne16_task_defs.h also defines (-Werror redefinition).
            #     OUR GAP9 target never added those SDK headers to `_includeList`
            #     (Deeploy/Targets/GAP9/Platform.py: pmsis.h, DeeployGAP9Math.h,
            #     pulp_nn_kernels.h, DeeployMchan.h), so the collision cannot occur and the
            #     workaround would only ADD headers (CNN_BasicKernels_fp32.h, CycleCounter.h)
            #     that our network lib does not put on the include path. Use the GAP9 default.
            # [upstream] cluster = GAP9ClusterEngine("GAP9Cluster", includeList = [
            # [upstream]     "pmsis.h", "DeeployGAP9Math.h", "pulp_nn_kernels.h", "DeeployMchan.h",
            # [upstream]     "CNN_BasicKernels_fp32.h", "CycleCounter.h"])
            cluster = GAP9ClusterEngine("GAP9Cluster")  # -- QW
